...
def pdf_url(x, pdf_local_url):
    ...
    fname_corrected = f"{fname_prefix}.{fname_suffix}"
...
def new_row(notion_connect, notion_table_id, zotrec):
    ...
    for key in relation_items:
        ...
        page_ids = [
            get_notion_pageID_by_pageName(notion_connect, notion_table_id, x[:-4])
            for x in zotrec[key]
        ]
...
def add_supp_dir(pdf_local_folder, supplementary_file_path):
    ...
    supplementary_file_path = os.path.abspath(
        os.path.expanduser(supplementary_file_path)
    )
...
def merge_IF_CAS(impact_factor, cas):
    ...
    return df.groupby("journal", as_index=False).agg(
        {"Subject": "; ".join, "Impact_factor": "first", "CAS": "first"}
    )


def select_attachment_items(items):
    items_with_attachment = []
    parent_keys = list({x["key"] for x in items if not x["data"].get("contentType")})
    attachment_keys = list(
        {x["key"] for x in items if x["data"].get("contentType") == "application/pdf"}
    )
    for item in items:
...
def get_relation_titles(zot, parent):
    ...
    if relation_info.get("dc:relation"):
        ...
        relation_titles = [
            get_children_title_from_parent_key(zot, x) for x in relation_keys
        ]
    else:
        relation_titles = []
...
def new_row_citekey(notion, notion_db_zotero_id, rec):
    props = {}
    page_id = get_notion_pageID_by_pageName(
        notion, notion_db_zotero_id, rec["pdf_name"][:-4]
    )
    if page_id != "":
...
def update_citekey(
    notion, notion_db_citekey_id, notion_db_zotero_id, bibtex, df_new_papers
):
    ...
    if len(bibs) > 0:
        ...
        for rec in bib_recs:
            ...
            if props != "":
                notion.pages.create(
                    parent={"database_id": notion_db_citekey_id}, properties=props
                )


@click.command()
@click.option(
    "--config", "-c", default="~/github/zotero2notion/config.ini", help="config.ini"
)
@click.option(
    "--zotero_topn",
    "-n",
    type=int,
    help="Fetch n most recent records in zotero to compare with the most recent records in Notion",
)
def main(config, zotero_topn):
    ...
    cfg.read(config)
    impact_factor = pkg_resources.resource_filename("zotero2notion", "if2021.tsv")
    cas = pkg_resources.resource_filename("zotero2notion", "cas2019.tsv")
    ...
    tbl["PDF"] = tbl.apply(
        lambda x: x["Local_file"].replace(pdf_local_url, pdf_remote_url), axis=1
    )
    tbl["Graph"] = tbl.apply(
        lambda x: url_connected_papers + urllib.parse.quote(x["Title"]), axis=1
    )

    # Fetch recent 100 records from notion table
    notion = Client(auth=notion_token)
    notion_records = notion.databases.query(
        **{
            "database_id": notion_table_id,
    ...
    if len(notion_records["results"]) == 0:
        print(
            "This is an empty notion database, please ensure you set columns properly"
        )
        # ------ TODO: set columns --------
    ...
    #         cv.collection.set(path=['schema'], value=schema)
        ...
        literature_existed = [
            ...
            for x in notion_records["results"]
        ]
        tbl = tbl[~tbl.Name.isin(literature_existed)]
        print(f"Updating {tbl.shape[0]} files...")
    ...
    if len(tbl) > 0:
        ...
        for rec in recs:
            try:
                ...
                notion.pages.create(
                    parent={"database_id": notion_table_id}, properties=props
                )
            except:
                print(props)
    else:
